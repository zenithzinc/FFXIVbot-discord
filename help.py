...
def getHelpMessage(action=None):
    if action == "주사위":
...
예: !선택 처치 텔레포 음식 쇠약 """]
    ...
    elif action in ["판매정보", "판매검색", "판매", "교환정보", "교환검색", "교환"]:
        return ["!" + action + " 아이템 이름 : 뒤에 입력한 아이템을 npc로부터 구할 수 있는지 표시합니다.",
                """아이템 이름은 띄어쓰기와 괄호 등을 정확히 입력해야 합니다. (완벽히 일치하지 않으면 검색되지 않습니다.)
예: !판매정보 하이 에테르"""]

    elif action in ["제작정보", "제작검색", "제작"]:
        return ["!" + action + " 아이템 이름 : 뒤에 입력한 아이템을 제작하는데 필요한 재료를 보여줍니다.",
                """아이템 이름은 띄어쓰기와 괄호 등을 정확히 입력해야 합니다. (완벽히 일치하지 않으면 검색되지 않습니다.)
예: !제작정보 하이 에테르"""]

    else:
        return ["FFXIV-ZnBot 기능 일람",
                """!판매정보 아이템이름, !제작정보 아이템이름, !주사위 M N, !선택 A B C...
!도움말 (기능) 을 입력하시면 기능별로 상세한 도움말을 보실 수 있습니다. (ex: !도움말 주사위)

기재되어있는 회사명·제품명·시스템 이름은 해당 소유자의 상표 또는 등록 상표입니다.
(C) 2010 - """ + str(datetime.now().year) + """ SQUARE ENIX CO. LTD All Rights Reserved.
Published in Korea by EYEDENTITY ENTERTAINMENT CO.,LTD.
문의: http://twitter.com/FFXIV_ZnBot, http://znzinc.net/ffxiv/znbot"""]
